...
@section('content')
        ...
        <div class="col-md-12">
            <form action="{{ route('artikel.update', $artikel->id_artikel) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" id="id_user" name="id_user" required>
                        ...
                        @foreach ($users as $user)
                            <option value="{{ $user->id }}" {{ $artikel->id_user == $user->id ? 'selected' : '' }}>
                                {{ $user->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" id="id_game" name="id_game" required>
                        ...
                        @foreach ($game as $games)
                            <option value="{{ $games->id_game }}"
                                {{ $artikel->id_game == $games->id_game ? 'selected' : '' }}>
                                {{ $games->nama_game }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    @if ($artikel->image)
                        <img src="{{ asset('storage/' . $artikel->image) }}" class="img-fluid mt-2" width="200">
                    @endif
                </div>

                <div class="form-group">
                    <label for="konten">Konten Artikel:</label>
                    <textarea class="form-control" name="konten" rows="5" required>{{ old('konten', $artikel->konten) }}</textarea>
                </div>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" name="status">
                        ...
                        <option value="pending" {{ $artikel->status == 'pending' ? 'selected' : '' }}>Pending (Kirim ke
                            Admin)</option>
                    </select>
                </div>
                    ...
                    @foreach ($artikelSections as $index => $section)
                        <div class="section-item">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" name="sections[{{ $index }}][sub_judul]"
                                    required value="{{ old('sections.' . $index . '.sub_judul', $section->sub_judul) }}">
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" name="sections[{{ $index }}][media_type]"
                                    ...
                                    <option value="image" {{ $section->media_type == 'image' ? 'selected' : '' }}>Image
                                    </option>
                                    <option value="embed" {{ $section->media_type == 'embed' ? 'selected' : '' }}>Embed
                                    </option>
                                </select>
                            </div>
                            ...
                            <div class="form-group" id="media-inputs-{{ $index }}"
                                ...
                                <input type="text" class="form-control d-none"
                                    ...
                                    value="{{ old('sections.' . $index . '.media_content', $section->media_content) }}">
                            </div>

                            <div class="form-group media-preview" id="media-preview-{{ $index }}">
                                @if ($section->media_type == 'image' && $section->media_content)
                                    <img src="{{ asset('storage/' . $section->media_content) }}" class="img-fluid rounded"
                                        width="200">
                                @elseif ($section->media_type == 'embed' && $section->media_content)
                                    <iframe width="300" height="200" src="{{ $section->media_content }}"
                            ...
                            <div class="form-group">
                                ...
                                <textarea class="form-control" name="sections[{{ $index }}][deskripsi]" rows="3" required>{{ old('sections.' . $index . '.deskripsi', $section->deskripsi) }}</textarea>
                            </div>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            let sectionIndex = {{ count($artikelSections) }}; // Mulai dari jumlah section yang ada
        ...
        function previewEmbed(index) {
            ...
            if (embedLink.includes("youtube.com") || embedLink.includes("youtu.be")) {
                mediaPreview.innerHTML =
                    `<iframe width="300" height="200" src="${embedLink.replace("watch?v=", "embed/")}" frameborder="0" allowfullscreen></iframe>`;
            } else {
